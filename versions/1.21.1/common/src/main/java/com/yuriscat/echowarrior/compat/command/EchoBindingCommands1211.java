...
/** Operator diagnostics and conservative recovery for persistent 1.21.1 summoner bindings. */
public final class EchoBindingCommands1211 {
    private EchoBindingCommands1211() {
    }
    ...
    private static int list(CommandSourceStack source) {
        ...
        int active = 0;
        int shown = 0;
        for (EchoBindingSavedData1211.Binding binding : EchoBindingSavedData1211.get(source.getServer()).bindings()) {
            total++;
            if (!binding.active()) continue;
            active++;
            if (shown++ < 20) source.sendSuccess(() -> shortStatus(binding), false);
        }
    ...
    private static int status(CommandSourceStack source, String rawUuid) {
        ...
        if (id == null) return 0;
        EchoBindingSavedData1211.Binding binding = EchoBindingSavedData1211.get(source.getServer()).get(id);
        if (binding == null) {
            source.sendFailure(Component.literal("未找到召唤器绑定：" + id));
            return 0;
        }
        source.sendSuccess(() -> shortStatus(binding), false);
        source.sendSuccess(() -> Component.literal("实体=" + value(binding.spiritId())
                + " 维度=" + binding.dimension() + " 生命=" + binding.health() + " 位置="
                + String.format(java.util.Locale.ROOT, "%.1f %.1f %.1f", binding.x(), binding.y(), binding.z())), false);
        source.sendSuccess(() -> Component.literal("generation=" + binding.generation()
                + " stateRevision=" + binding.stateRevision() + " fuel=" + binding.fuel()), false);
        return 1;
    }
    ...
    private static int dismiss(CommandSourceStack source, String rawUuid) {
        UUID id = parse(source, rawUuid);
        if (id == null) return 0;
        if (!EchoBindingSystem1211.dismiss(source.getServer(), id)) {
    ...
    private static int repair(CommandSourceStack source, String rawUuid) {
        UUID id = parse(source, rawUuid);
        if (id == null) return 0;
        if (!EchoBindingSystem1211.forceReconstruct(source.getServer(), id)) {
            source.sendFailure(Component.literal("无法重建：绑定必须活跃、处于跟随状态，且控制者在线并持有召唤器。"));
            return 0;
        }
    ...
    private static Component shortStatus(EchoBindingSavedData1211.Binding binding) {
        ItemStack relic = binding.relic();
        Component hero = relic.getItem() instanceof EchoRelicItem1211
                ? Component.translatable(EchoHeroType1211.fromRelic(relic).translationKey())
