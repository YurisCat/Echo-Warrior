 ...
 */
public final class EchoActivityMovement1211 {
	...
	private EchoActivityMovement1211() {
	}

	public static void reset(PathfinderMob echo) {
		MOVEMENT_STATES.remove(echo);
		echo.getNavigation().stop();
		BrainUtils.clearMemory(echo, MemoryModuleType.WALK_TARGET);
	}

	public static void tick(ServerLevel level, PathfinderMob echo, EchoRelicState1211.ActivityMode mode,
		...
		if (mode == EchoRelicState1211.ActivityMode.FOLLOW) {
		...
		if (mode == EchoRelicState1211.ActivityMode.WAIT) {
	...
	private static final class MovementState {
		private EchoRelicState1211.ActivityMode mode;
		...
		private static MovementState entering(EchoRelicState1211.ActivityMode mode, Vec3 position,
		...
		private void enter(EchoRelicState1211.ActivityMode mode, Vec3 position, int pauseTicks, long now) {
			this.mode = mode;
