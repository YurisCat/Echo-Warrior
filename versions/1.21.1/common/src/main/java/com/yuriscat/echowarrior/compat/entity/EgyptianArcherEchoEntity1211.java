...
public final class EgyptianArcherEchoEntity1211 extends PathfinderMob implements EchoWarriorEntity1211, GeoEntity {
	private static final boolean RANGED_ANIMATION_DIAGNOSTICS =
			Boolean.getBoolean("echo_warrior.egyptian_archer_diagnostics");
	...
	private static final EntityDataAccessor<Float> ATTENTION_X = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> ATTENTION_Y = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> ATTENTION_Z = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> EYE_ATTENTION_X = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> EYE_ATTENTION_Y = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> EYE_ATTENTION_Z = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Byte> VISUAL_REACTION = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.BYTE);
	private static final EntityDataAccessor<Long> VISUAL_REACTION_UNTIL = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.LONG);
	private static final EntityDataAccessor<Long> BLINK_START = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.LONG);
	private static final EntityDataAccessor<Byte> BLINK_COUNT = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.BYTE);
	private static final EntityDataAccessor<Byte> CURIOUS_TILT = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.BYTE);
	private static final EntityDataAccessor<Integer> VISUAL_SEQUENCE = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.INT);
	private static final EntityDataAccessor<Long> ATTENTION_STARTED_AT = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.LONG);
	private static final EntityDataAccessor<Long> CAUGHT_REACTION_START = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.LONG);
	private static final EntityDataAccessor<Boolean> COMBAT_GAZE_LOCKED = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> COMBAT_GAZE_TARGET_ID = SynchedEntityData.defineId(EgyptianArcherEchoEntity1211.class, EntityDataSerializers.INT);
	...
	private static final EntityDataAccessor<Byte> ACTION = SynchedEntityData.defineId(
			EgyptianArcherEchoEntity1211.class, EntityDataSerializers.BYTE);
	private static final EntityDataAccessor<Integer> ATTACK_INTERVAL = SynchedEntityData.defineId(
			EgyptianArcherEchoEntity1211.class, EntityDataSerializers.INT);
	...
	private static final EntityDataAccessor<Boolean> RANGED_RELOAD_STYLE = SynchedEntityData.defineId(
			EgyptianArcherEchoEntity1211.class, EntityDataSerializers.BOOLEAN);
	...
	private static final RawAnimation FIRST_DRAW_SEQUENCE_UPPER = RawAnimation.begin().thenPlayAndHold("animation.egyptian_archer.draw_bow_upper");
	private static final RawAnimation RELOAD_DRAW_SEQUENCE_UPPER = RawAnimation.begin().thenPlayAndHold("animation.egyptian_archer.reload_bow_upper");
	...
	private static final int COMBAT_SIGHT_MEMORY_TICKS = EchoTargetVisibilityMemory1211.GRACE_TICKS;
	...
	private static final float FIRST_DRAW_SEQUENCE_ANIMATION_SECONDS = 2.25F;
	private static final float RELOAD_DRAW_SEQUENCE_ANIMATION_SECONDS = 2.23333F;
	private static final float RELEASE_ANIMATION_SECONDS = 0.25F;
	private static final float UNNOCK_ANIMATION_SECONDS = 1.69166F;
	...
	private LivingEntity attentionTarget;
	...
	private LivingEntity eyeAttentionTarget;
	...
	private UUID mutualGazePlayerUuid;
	...
	private int enabledSkills = EchoHeroType1211.EGYPTIAN_ARCHER.defaultEnabledSkillsMask();
	private final EchoCreeperTargeting1211 creeperTargeting = new EchoCreeperTargeting1211();
	private EchoRelicState1211.EgyptianArrowMode arrowMode = EchoRelicState1211.EgyptianArrowMode.OFF;
	...
	private UUID aimTargetUuid;
	private Entity committedAimTarget;
	private boolean pendingBackstep;
	private long targetLostAt = -1L;
	private LivingEntity actionTarget;
	private LivingEntity combatSightTarget;
	...
	private Monster highThreatTarget;
	private LivingEntity resumeTargetAfterThreat;
	private LivingEntity emergencyTarget;
	private LivingEntity resumeTargetAfterEmergency;
	...
	private LivingEntity combatGazeTarget;
	...
	private AttentionKind lastLoggedPostCombatAttentionKind;
	...
	public EgyptianArcherEchoEntity1211(EntityType<? extends EgyptianArcherEchoEntity1211> type, Level level) {
		super(type, level);
	...
	public static AttributeSupplier.Builder createAttributes() {
		return PathfinderMob.createMobAttributes()
				.add(Attributes.MAX_HEALTH, EchoHeroType1211.EGYPTIAN_ARCHER.baseMaximumHealth())
				.add(Attributes.ARMOR, EchoHeroType1211.EGYPTIAN_ARCHER.baseArmor())
				.add(Attributes.ATTACK_DAMAGE, EchoHeroType1211.EGYPTIAN_ARCHER.baseAttackDamage())
				.add(Attributes.MOVEMENT_SPEED, EchoHeroType1211.EGYPTIAN_ARCHER.baseMovementSpeed())
				.add(Attributes.FOLLOW_RANGE, 32.0)
				.add(Attributes.KNOCKBACK_RESISTANCE, EchoHeroType1211.EGYPTIAN_ARCHER.baseKnockbackResistance());
	}
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		...
		builder.define(ATTACK_INTERVAL, EchoHeroType1211.EGYPTIAN_ARCHER.baseAttackIntervalTicks());
	...
	public void aiStep() {
		...
		if (!relic.isEmpty()) {
			EchoRelicState1211.backstepCharges(relic, level.getGameTime());
		...
		CatGodCreeperSystem1211.tickAura(level, this);
		...
		EchoWaterSafety1211.tick(level, this, owner, controllerAvailable && this.activityMode == EchoRelicState1211.ActivityMode.FOLLOW
	...
	private void tickNock(ServerLevel level, long now, ItemStack relic, LivingEntity target) {
	...
	private void tickDraw(ServerLevel level, long now, ItemStack relic, LivingEntity target) {
		if (canContinueCombatAgainst(target)) {
			this.targetLostAt = -1L;
			this.actionTarget = target;
			this.getLookControl().setLookAt(target, 35.0F, 35.0F);
	...
	private void tickAim(ServerLevel level, long now, ItemStack relic, LivingEntity target) {
		if (!canContinueCombatAgainst(target)) {
			this.setTarget(null);
			stopMovementIntent();
			if (this.targetLostAt < 0L) this.targetLostAt = now;
	...
	private void tickCommittedMelee(ServerLevel level, long now, ItemStack relic) {
		...
		if (!this.shotReleased && now >= this.shotReleaseAt) {
			...
			if (eligible) {
				...
				damaged = meleeTarget.hurt(level.damageSources().mobAttack(this), damage);
				if (damaged) {
	...
	private void startNock(long now, LivingEntity target, boolean reloadStyle) {
		...
		logRangedAnimationPhase("nock_start", now);
		EchoWarrior1211.LOGGER.debug(
	...
	private void startDraw(long now, LivingEntity target) {
		...
		this.actionTarget = target;
		this.targetLostAt = -1L;
		// GeckoLib 4 resets a triggered controller whenever a second clip replaces it.
		// Keep nocking and drawing in the authored continuous sequence so the arms do
		// not snap back between the two server-side combat phases.
		logRangedAnimationPhase("draw_start", now);
	}
	...
	private void startAim(long now, LivingEntity target) {
		...
		this.aimTicksRemaining = this.rangedPhaseBudget.aimTicks();
		this.aimTargetUuid = target.getUUID();
		// The combined nock/draw trigger already holds the authored full-draw
		// final frame. Starting the one-keyframe looping aim clip here makes
		// GeckoLib 4 re-enter its transition on every loop, which visibly snaps
		// the hands back while the archer is aiming.
		logRangedAnimationPhase("aim_start", now);
	}
	...
	private void startRelease(long now, LivingEntity target) {
		...
		logRangedAnimationPhase("release_start", now);
	}

	private void logRangedAnimationPhase(String event, long now) {
		if (!RANGED_ANIMATION_DIAGNOSTICS) return;
		RangedPhaseBudget budget = rangedPhaseBudget(attackInterval());
		float sourceSeconds = this.entityData.get(RANGED_RELOAD_STYLE)
				? RELOAD_DRAW_SEQUENCE_ANIMATION_SECONDS : FIRST_DRAW_SEQUENCE_ANIMATION_SECONDS;
		double sequenceSpeed = sourceSeconds * 20.0F
				/ Math.max(1, budget.nockTicks() + budget.drawTicks());
		EchoWarrior1211.LOGGER.info(
				"[EgyptianArcherAnimationServer] archer={} tick={} event={} action={} interval={} "
						+ "budget={}/{}/{}/{} duration={} reload={} sequenceSpeed={}",
				this.getId(), now, event, actionName(action()), attackInterval(), budget.nockTicks(),
				budget.drawTicks(), budget.aimTicks(), budget.releaseTicks(),
				this.entityData.get(ACTION_DURATION_TICKS), this.entityData.get(RANGED_RELOAD_STYLE), sequenceSpeed);
	}

	private void fireMainShot(ServerLevel level, LivingEntity target, Entity aimTarget) {
		...
		if (this.random.nextFloat() < chance
			...
			level.sendParticles(dust(VOLLEY_PARTICLE_COLOR, 0.9F),
					this.getX(), this.getY() + this.getBbHeight() * 0.58, this.getZ(),
					14, 0.42, 0.55, 0.42, 0.025);
		}
	}

	private EgyptianArcherArrowEntity1211 spawnArrow(ServerLevel level, LivingEntity target) {
		Entity aimTarget = rangedAimTarget(target);
	...
	private EgyptianArcherArrowEntity1211 spawnArrow(ServerLevel level, LivingEntity target,
		...
		EgyptianArcherArrowEntity1211 arrow = ModContent1211.EGYPTIAN_ARCHER_ARROW.create(level);
		if (arrow == null) return null;
		arrow.setPos(this.getX(), this.getEyeY() - 0.18, this.getZ());
		boolean pierce = this.arrowMode == EchoRelicState1211.EgyptianArrowMode.CONE && this.random.nextFloat() < 0.25F;
	...
	private boolean canFireCommittedShot(LivingEntity target) {
	...
	private boolean isCommittedTargetAlive(LivingEntity target) {
		return target != null && target.isAlive() && !target.isRemoved() && this.canAttack(target);
	}

	private Entity committedAimTarget(LivingEntity target) {
	...
	private double aimAlignmentDot(Entity aimTarget) {
	...
	private boolean isCombatSightExpired(LivingEntity target) {
		return target != null && target == this.combatSightTarget && this.combatSightExpired;
	}

	private boolean canAcquireCombatTarget(LivingEntity target) {
	...
	private Entity rangedAimTarget(LivingEntity target) {
	...
	private boolean canLandCommittedMeleeHit(LivingEntity target) {
		return canDefendAgainst(target)
	...
	private LivingEntity selectImmediateMeleeThreat(ServerLevel level, LivingEntity currentTarget) {
		LivingEntity owner = this.getOwner();
	...
	private LivingEntity selectCommittedMeleeReplacement(ServerLevel level,
	...
	private LivingEntity nearestMeleeEscapeThreat(ServerLevel level,
	...
	private boolean canBackstep(ItemStack relic) {
		return !relic.isEmpty() && EchoRelicState1211.skillEnabled(relic, SKILL_BACKSTEP)
				&& EchoRelicState1211.backstepCharges(relic, this.level().getGameTime()) > 0;
	}
	...
	private boolean startBackstep(ServerLevel level, ItemStack relic, LivingEntity nearestEnemy) {
		...
		if (landing == null || !EchoRelicState1211.consumeBackstepCharge(relic, level.getGameTime())) return false;
		stopBowAnimations();
		clearMeleeEscape();
	...
	private void tickBackstep(ServerLevel level, ItemStack relic) {
		...
		if (!isBackstepPointSafe(level, point)) {
			this.snapCompat(this.backstepLastSafe.x, this.backstepLastSafe.y, this.backstepLastSafe.z,
					this.backstepYaw, this.getXRot());
			this.setNoGravity(false);
			finishAction();
			return;
		}
		this.snapCompat(point.x, point.y, point.z, this.backstepYaw, this.getXRot());
		...
		if (elapsed >= BACKSTEP_TICKS) {
			this.snapCompat(this.backstepLanding.x, this.backstepLanding.y, this.backstepLanding.z,
					this.backstepYaw, this.getXRot());
			this.setNoGravity(false);
			finishAction();
		}
	...
	private Vec3 findBackstepLanding(ServerLevel level, LivingEntity nearestEnemy) {
		Vec3 best = null;
	...
	private List<LivingEntity> combatTargets(ServerLevel level, double range, LivingEntity excluded) {
		long now = level.getGameTime();
	...
	private void tickMovement(ServerLevel level, LivingEntity owner) {
		...
		resetCombatRetreat();
		if (this.activityMode == EchoRelicState1211.ActivityMode.FOLLOW) {
			double distance = this.distanceTo(owner);
			if (distance > 32.0) {
				stopMovementIntent();
				if (!EchoSafeTeleport1211.isStableBelowSuspendedOwner(this, owner)
						&& EchoSafeTeleport1211.mayAttemptAutomaticTeleport(level, this)) {
					recallTo((Player)owner);
				}
			} else if (distance > 5.0) {
				if (EchoSafeTeleport1211.shouldSuspendGroundFollow(this, owner)) stopMovementIntent();
				else this.getNavigation().moveTo(owner, 1.0);
			}
	...
	private List<LivingEntity> meleeRetreatThreats(ServerLevel level, LivingEntity primaryTarget) {
		LivingEntity recentAttacker = this.getLastHurtByMob();
		...
		List<LivingEntity> threats = new ArrayList<>(level.getEntitiesOfClass(LivingEntity.class,
				this.getBoundingBox().inflate(CLOSE_THREAT_TRIGGER_RANGE), candidate ->
						candidate != this && candidate.distanceToSqr(this) <= CLOSE_THREAT_TRIGGER_RANGE * CLOSE_THREAT_TRIGGER_RANGE
								...
								&& (!(candidate instanceof Creeper creeper) || !CatGodCreeperSystem1211.isPanicking(creeper)
										|| recentDamage && candidate == recentAttacker)));
	...
	private RetreatPath findRetreatPath(ServerLevel level, List<LivingEntity> threats) {
	...
	private RetreatPath findRetreatPath(ServerLevel level, List<LivingEntity> threats, Vec3 away,
	...
	private LivingEntity resolveCombatGazeTarget(long now) {
		if (action() == ACTION_SHOOT && isCommittedTargetAlive(this.actionTarget)) {
	...
	private boolean canLockCombatGazeTo(LivingEntity target) {
	...
	private void logCombatGazeTransition(long now, LivingEntity resolvedTarget, boolean locked) {
	...
	private Player tickPlayerGazeAcquisition(ServerLevel level, LivingEntity owner, boolean combatSuppressed, long now) {
	...
	private boolean tickMutualGaze(ServerLevel level, long now, AttentionCandidate candidate, boolean attentionScanTick) {
	...
	private static boolean sameTarget(LivingEntity first, LivingEntity second) {
		return first == second;
	}
	...
	public static boolean isRangedAnimationDiagnosticsEnabled() {
		return RANGED_ANIMATION_DIAGNOSTICS;
	}

	public int getActionDurationTicksForDiagnostics() {
		return this.entityData.get(ACTION_DURATION_TICKS);
	}

	public int getAttackIntervalForDiagnostics() {
		return attackInterval();
	}

	public boolean isReloadStyleForDiagnostics() {
		return this.entityData.get(RANGED_RELOAD_STYLE);
	}

	public String getActionNameForDiagnostics() {
	...
	private LivingEntity selectProtectiveTarget(LivingEntity owner) {
		...
		if (this.alertMode != EchoRelicState1211.AlertMode.AGGRESSIVE) return null;
		double range = EchoAccessorySystem1211.proactiveRange(this, MAX_RANGE,
	...
	private LivingEntity selectVisibleCombatReplacement(LivingEntity owner, LivingEntity hiddenCurrent) {
		long now = this.level().getGameTime();
		LivingEntity ownAttacker = this.getLastHurtByMob();
	...
	private LivingEntity clearExpiredEmergencyTarget(LivingEntity current) {
	...
	private Monster selectBoundaryIndependentSelfDefenseTarget(LivingEntity owner,
			LivingEntity current, LivingEntity retainedSelfDefense) {
		LivingEntity ownAttacker = this.getLastHurtByMob();
		LivingEntity ownerAttacker = owner.getLastHurtByMob();
		...
		List<Monster> candidates = this.level().getEntitiesOfClass(Monster.class,
				this.getBoundingBox().inflate(BACKSTEP_TRIGGER_RANGE), candidate -> {
					...
					if (this.alertMode == EchoRelicState1211.AlertMode.PEACEFUL) return ownDamager;
					return this.alertMode == EchoRelicState1211.AlertMode.AGGRESSIVE
	...
	private Monster selectCloseThreat(LivingEntity owner, LivingEntity current,
			boolean allowLingeringThreat) {
		boolean alreadyInCombat = canContinueCombatAgainst(current)
				|| this.alertMode == EchoRelicState1211.AlertMode.AGGRESSIVE;
		LivingEntity ownAttacker = this.getLastHurtByMob();
		LivingEntity ownerAttacker = owner.getLastHurtByMob();
		boolean ownDamageRecent = isRecent(this, this.getLastHurtByMobTimestamp());
		boolean ownerDamageRecent = isRecent(owner, owner.getLastHurtByMobTimestamp());
		...
		candidates.removeIf(candidate -> {
					...
					if (this.alertMode == EchoRelicState1211.AlertMode.PEACEFUL
	...
	private int closeThreatPriority(Monster candidate, LivingEntity owner,
			LivingEntity ownAttacker, boolean ownDamageRecent,
			LivingEntity ownerAttacker, boolean ownerDamageRecent) {
	...
	private void refreshReactiveCreeperPermission(LivingEntity owner, long now) {
		...
		if (this.alertMode == EchoRelicState1211.AlertMode.PEACEFUL) return;
		LivingEntity ownerAttacker = owner.getLastHurtByMob();
		if (isRecentWithin(owner, owner.getLastHurtByMobTimestamp(), EchoTargetVisibilityMemory1211.GRACE_TICKS)
	...
	private boolean canContinueCombatAgainst(LivingEntity target) {
	...
	private boolean isRetainedSelfDefenseTarget(LivingEntity target) {
	...
	private LivingEntity validateSelfDefenseTarget() {
	...
	private void retainSelfDefenseTarget(LivingEntity target, String reason) {
		if (!canDefendAgainst(target)
				|| !this.creeperTargeting.canTarget(this, target, this.level().getGameTime(),
						allowProactiveCreeperTargeting())
	...
	private boolean canDefendAgainst(LivingEntity target) {
	...
	public boolean hurt(DamageSource source, float damage) {
		...
		if (attacker == this.getOwner() || attacker instanceof EchoWarriorEntity1211 echo && echo.ownerEntity() == this.getOwner()) return false;
		long now = level.getGameTime();
	...
	public void applyRelicState(ItemStack relic, boolean resetAnchor) {
		...
		this.arrowMode = EchoRelicState1211.egyptianArrowMode(relic);
		this.entityData.set(ARROW_MODE, this.arrowMode.ordinal());
		this.entityData.set(ATTACK_INTERVAL, EchoTalentSystem1211.attackIntervalTicks(this, relic));
		if (previousActivity != this.activityMode || previousAlert != this.alertMode || resetAnchor) {
			...
			this.resumeTargetAfterEmergency = null;
			this.emergencyTargetUntil = 0L;
			EchoActivityMovement1211.reset(this);
		}
	...
	public void readMigrationState(CompoundTag tag) {
		this.lastNaturalHealAt = tag.getLong("EgyptianLastNaturalHealAt");
	}
	...
	@Override public EchoHeroType1211 heroType() { return EchoHeroType1211.EGYPTIAN_ARCHER; }
	@Override public boolean shouldFollowOwner() {
		return this.activityMode == EchoRelicState1211.ActivityMode.FOLLOW
				&& action() != ACTION_BACKSTEP && !isVisualInteractionMovementOwned();
	}
	...
	public void readAdditionalSaveData(CompoundTag tag) {
		...
		this.enabledSkills = tag.contains("EnabledSkills") ? tag.getInt("EnabledSkills")
				: EchoHeroType1211.EGYPTIAN_ARCHER.defaultEnabledSkillsMask();
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
		controllers.add(new AnimationController<EgyptianArcherEchoEntity1211>(this, "movement", 3, this::selectMovementAnimation));
		// Every authored bow boundary is already pose-continuous: full draw ends at
		// exactly the first shoot frame, and shoot ends at exactly the first reload
		// frame. GeckoLib 4's extra one-tick transition briefly re-samples the held
		// controller pose at release, which presents as a hand twitch on NeoForge.
		AnimationController<EgyptianArcherEchoEntity1211> actionController =
				new AnimationController<EgyptianArcherEchoEntity1211>(this, ACTION_CONTROLLER, 0, this::selectActionAnimation);
		actionController
				.setAnimationSpeedHandler(archer -> actionAnimationSpeed(archer, actionController))
				.triggerableAnim(FIRST_NOCK_TRIGGER, FIRST_DRAW_SEQUENCE_UPPER)
				.triggerableAnim(RELOAD_NOCK_TRIGGER, RELOAD_DRAW_SEQUENCE_UPPER)
	...
	private PlayState selectMovementAnimation(AnimationState<EgyptianArcherEchoEntity1211> test) {
		EgyptianArcherEchoEntity1211 archer = test.getAnimatable();
	...
	private PlayState selectActionAnimation(AnimationState<EgyptianArcherEchoEntity1211> test) {
		return PlayState.STOP;
	}

	private static double actionAnimationSpeed(EgyptianArcherEchoEntity1211 archer,
			AnimationController<EgyptianArcherEchoEntity1211> controller) {
		RawAnimation triggeredAnimation = controller.getTriggeredAnimation();
		float sourceSeconds;
		int durationTicks;
		if (triggeredAnimation == FIRST_DRAW_SEQUENCE_UPPER || triggeredAnimation == RELOAD_DRAW_SEQUENCE_UPPER) {
			sourceSeconds = triggeredAnimation == RELOAD_DRAW_SEQUENCE_UPPER
					? RELOAD_DRAW_SEQUENCE_ANIMATION_SECONDS : FIRST_DRAW_SEQUENCE_ANIMATION_SECONDS;
			RangedPhaseBudget budget = rangedPhaseBudget(archer.attackInterval());
			durationTicks = budget.nockTicks() + budget.drawTicks();
		} else if (triggeredAnimation == SHOOT_UPPER) {
			sourceSeconds = RELEASE_ANIMATION_SECONDS;
			durationTicks = rangedPhaseBudget(archer.attackInterval()).releaseTicks();
		} else if (triggeredAnimation == UNNOCK_UPPER) {
			sourceSeconds = UNNOCK_ANIMATION_SECONDS;
			durationTicks = UNNOCK_TICKS;
		} else {
			return 1.0F;
		}
		// GeckoLib 4 calculates animation time as currentSpeed * totalElapsedTime.
		// Selecting speed from the synchronized action made the NOCK/DRAW sequence
		// jump backwards when ACTION changed to AIM. Keep the speed attached to the
		// triggered clip itself so phase synchronization cannot rewind its timeline.
		// Derive release/unnock duration from stable constants as well: the trigger
		// packet can arrive one render frame before synchronized ACTION_DURATION_TICKS.
		if (sourceSeconds <= 0.0F) return 1.0F;
		return sourceSeconds * 20.0F / Math.max(1, durationTicks);
	}

	@Override public AnimatableInstanceCache getAnimatableInstanceCache() { return this.animationCache; }

	private void snapCompat(double x, double y, double z, float yaw, float pitch) {
		this.setPos(x, y, z);
		this.setYRot(yaw);
