 ...
 */
final class GuandaoVisualBehavior1211<T extends PathfinderMob & GuandaoVisualBehavior1211.Host> {
	...
	GuandaoVisualBehavior1211(T entity) {
		this.entity = entity;
	}
	...
	void onHurt(long now, LivingEntity attacker) {
		endCaughtExit(now, true);
	...
	private Player tickPlayerGazeAcquisition(ServerLevel level, LivingEntity owner, boolean suppressed) {
	...
	private boolean tickMutualGaze(ServerLevel level, long now, AttentionCandidate candidate, boolean scanTick) {
