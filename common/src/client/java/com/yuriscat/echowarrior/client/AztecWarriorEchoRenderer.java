...
public final class AztecWarriorEchoRenderer extends GeoEntityRenderer<AztecWarriorEchoEntity, EntityRenderState> {
	...
	private static final DataTicket<Integer> ENTITY_ID = DataTickets.create("echo_warrior_aztec_entity_id", Integer.class);
	private static final DataTicket<Vec3> ENTITY_POSITION = DataTickets.create("echo_warrior_aztec_entity_position", Vec3.class);
	private static final DataTicket<Vec3> ATTENTION_POINT = DataTickets.create("echo_warrior_aztec_attention_point", Vec3.class);
	private static final DataTicket<Vec3> EYE_ATTENTION_POINT = DataTickets.create("echo_warrior_aztec_eye_attention_point", Vec3.class);
	private static final DataTicket<Float> BODY_YAW = DataTickets.create("echo_warrior_aztec_body_yaw", Float.class);
	private static final DataTicket<Byte> REACTION = DataTickets.create("echo_warrior_aztec_reaction", Byte.class);
	private static final DataTicket<Long> GAME_TIME = DataTickets.create("echo_warrior_aztec_game_time", Long.class);
	private static final DataTicket<Long> BLINK_START = DataTickets.create("echo_warrior_aztec_blink_start", Long.class);
	private static final DataTicket<Byte> BLINK_COUNT = DataTickets.create("echo_warrior_aztec_blink_count", Byte.class);
	private static final DataTicket<Byte> CURIOUS_TILT = DataTickets.create("echo_warrior_aztec_curious_tilt", Byte.class);
	private static final DataTicket<Integer> VISUAL_SEQUENCE = DataTickets.create("echo_warrior_aztec_visual_sequence", Integer.class);
	private static final DataTicket<Long> ATTENTION_STARTED_AT = DataTickets.create("echo_warrior_aztec_attention_started_at", Long.class);
	private static final DataTicket<Long> CAUGHT_REACTION_START = DataTickets.create("echo_warrior_aztec_caught_reaction_start", Long.class);
	...
	@Override
	public void captureDefaultRenderState(AztecWarriorEchoEntity entity, Void relatedObject, EntityRenderState renderState, float partialTick) {
	...
	public void adjustModelBonesForRender(RenderPassInfo<EntityRenderState> renderPass, BoneSnapshots snapshots) {
		...
		boolean locomotionGaze = reaction == AztecWarriorEchoEntity.VISUAL_LOCOMOTION;
		...
		float desiredTilt = !shieldRaised && reaction == AztecWarriorEchoEntity.VISUAL_CURIOUS
		...
		} else if (reaction == AztecWarriorEchoEntity.VISUAL_STARTLED || reaction == AztecWarriorEchoEntity.VISUAL_HURT) {
			headResponsiveness = 0.55F;
		} else if (reaction == AztecWarriorEchoEntity.VISUAL_CAUGHT) {
			headResponsiveness = 0.36F;
		} else if (reaction == AztecWarriorEchoEntity.VISUAL_MUTUAL_GAZE) {
		...
		float eyeResponsiveness = reaction == AztecWarriorEchoEntity.VISUAL_STARTLED || reaction == AztecWarriorEchoEntity.VISUAL_HURT
				? 0.92F
				: reaction == AztecWarriorEchoEntity.VISUAL_CAUGHT ? 0.9F
				: reaction == AztecWarriorEchoEntity.VISUAL_MUTUAL_GAZE ? 0.82F : 0.58F;
		...
		float desiredPupilScale = switch (reaction) {
			case AztecWarriorEchoEntity.VISUAL_HURT -> 0.6F;
			case AztecWarriorEchoEntity.VISUAL_STARTLED -> 0.48F;
			case AztecWarriorEchoEntity.VISUAL_CAUGHT -> Mth.lerp(
		...
		float blink = reaction == AztecWarriorEchoEntity.VISUAL_STARTLED
				? 0.0F
				: reaction == AztecWarriorEchoEntity.VISUAL_HURT
