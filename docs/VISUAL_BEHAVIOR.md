...
The Roman legionary, Aztec warrior, and Egyptian archer echoes use the same code-driven visual-attention layer on top of GeckoLib locomotion and combat animations. This layer is cosmetic: it must not change navigation, combat targeting, damage, or summoner binding. Their timing, probability, priority, and interruption rules are intentionally identical; only model-specific bone names and torso ancestry differ. The Egyptian archer keeps blink and expression reactions during draw, shoot, melee, and backstep actions, but any valid combat target hard-locks the synced head and pupil attention points to the current shot target (or the selected Ender Dragon part), clears curious tilt, suppresses social/environmental gaze, and lets combat aiming own the body facing. Equal-priority close threats remain sticky instead of stealing focus by tiny distance changes. A strictly higher-danger threat may redirect an in-progress draw without restarting its animation phase, and release waits until body, head, and pupils have turned back within the firing tolerance; once `SHOOT` begins, its committed target remains frozen until that release completes unless point-blank self-defence takes over. A committed combat action keeps using its action target, plus a three-tick grace period, if the general target field is briefly empty at a shot boundary. Close self-defence keeps the same threat committed through melee, backstep, and post-landing re-aim until it dies, becomes invalid, or moves more than ten blocks away, even outside a WAIT/WANDER activity anchor; it may retreat, hold aim, and retaliate, but may not path toward that out-of-bound target when line of sight is lost. The same commitment keeps combat gaze locked, so ordinary Creeper alert and environmental attention cannot steal the head during the escape. When the final target disappears, a 30-tick post-combat settle keeps attention on the last body-forward direction and hands directly to locomotion if movement begins; default owner attention is withheld until the settle ends, preventing the owner → locomotion → owner double turn. Ordinary non-combat head turns use a gentler response after this handoff. While combat lock is active, the renderer composes the real `Main` → `Upper_Body` → `Upper_Body2` rotation and applies its exact inverse to `Head`; multi-axis draw and reload poses must not be approximated by subtracting Euler components. Hurt and threat reactions may change the facial expression but must not redirect the head or pupils away from that target. Transition-only server diagnostics use `[EgyptianArcherGaze]`; close self-defence acquisition and release use `[EgyptianArcherSelfDefense]`; `[EgyptianArcherPostCombatGaze]` records attention-kind changes only in a short post-combat window. The rate-limited `[EgyptianArcherGazeClient]` diagnostic derives the final rendered face-forward vector from the actual world positions of `Head`, `Eyes`, and `Hat`, and reports sustained world-space angular error rather than local Euler compensation jumps.
