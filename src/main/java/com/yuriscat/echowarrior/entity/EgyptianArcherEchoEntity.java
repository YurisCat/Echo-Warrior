...
public final class EgyptianArcherEchoEntity extends PathfinderMob implements EchoWarriorEntity, GeoEntity {
	...
	private void tickMovement(ServerLevel level, LivingEntity owner) {
		...
		if (this.activityMode == EchoRelicState.ActivityMode.FOLLOW) {
			...
			if (distance > 32.0) {
				stopMovementIntent();
				if (!EchoSafeTeleport.isStableBelowSuspendedOwner(this, owner)
						&& EchoSafeTeleport.mayAttemptAutomaticTeleport(level, this)) {
					recallTo((Player)owner);
				}
			} else if (distance > 5.0) {
				if (EchoSafeTeleport.shouldSuspendGroundFollow(this, owner)) stopMovementIntent();
