...
public final class BattlefieldSavedData extends SavedData {
	...
	private static final Codec<BattlefieldSavedData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
			RegionState.CODEC.listOf().optionalFieldOf("regions", List.of()).forGetter(data -> List.copyOf(data.regions.values())),
			CompletedSite.CODEC.listOf().optionalFieldOf("completed_sites", List.of()).forGetter(data -> data.completedSites),
	...
	private BattlefieldSavedData(List<RegionState> regions, List<CompletedSite> completedSites, long lastPlacementTick) {
	...
	public RegionState noteNaturalChunkLoad(ServerLevel level, ChunkPos chunkPos, boolean newlyGenerated) {
		...
		if (current == null) {
			...
			current = new RegionState(regionX, regionZ, Status.WAITING, BlockPos.ZERO.asLong(),
					BlockPos.ZERO.asLong(), now + delay, chunkPos.pack(), level.getRandom().nextLong());
	...
	public void activate(long key, BlockPos center, BlockPos relic) {
		RegionState state = this.regions.get(key);
		if (state == null) return;
		this.regions.put(key, state.active(center.asLong(), relic.asLong()));
		setDirty();
	}

	public boolean completeAt(BlockPos relic, long now) {
		for (Map.Entry<Long, RegionState> entry : this.regions.entrySet()) {
			RegionState state = entry.getValue();
			if (state.status != Status.ACTIVE || state.relicPos != relic.asLong()) continue;
			this.completedSites.add(new CompletedSite(state.centerPos, now));
	...
	public boolean isFarEnoughFromKnownSites(BlockPos center, double minimumDistance) {
		...
		for (RegionState state : this.regions.values()) {
			if (state.status == Status.ACTIVE && horizontalDistanceSqr(BlockPos.of(state.centerPos), center) < minimumSqr) return false;
		}
	...
	public record ActiveSite(BlockPos center, BlockPos relic) {
	}
	...
	public record RegionState(
		...
		public long key() { return ChunkPos.pack(this.regionX, this.regionZ); }
		public RegionState withCandidateChunk(long candidate) {
			return new RegionState(this.regionX, this.regionZ, this.status, this.centerPos, this.relicPos,
					this.readyAt, candidate, this.seed);
		}
		public RegionState withReadyAt(long tick) {
			return new RegionState(this.regionX, this.regionZ, this.status, this.centerPos, this.relicPos,
					tick, this.candidateChunk, this.seed);
		}
		public RegionState waiting(long now, long candidate, long newSeed) {
			return new RegionState(this.regionX, this.regionZ, Status.WAITING, this.centerPos, this.relicPos,
					now, candidate, newSeed);
		}
		public RegionState active(long center, long relic) {
			return new RegionState(this.regionX, this.regionZ, Status.ACTIVE, center, relic,
					Long.MAX_VALUE, this.candidateChunk, this.seed);
		}
		public RegionState cooldown(long until) {
			return new RegionState(this.regionX, this.regionZ, Status.COOLDOWN, this.centerPos, this.relicPos,
					until, this.candidateChunk, this.seed);
		}
		public ActiveSite activeSite() { return new ActiveSite(BlockPos.of(this.centerPos), BlockPos.of(this.relicPos)); }
	}

	private record CompletedSite(long centerPos, long completedAt) {
