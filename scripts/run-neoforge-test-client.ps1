[CmdletBinding()]
param(
    [string]$TestWorldName = 'CATTEST_NEOFORGE',
...
$worldPath = Join-Path $projectRoot "run-neoforge\saves\$TestWorldName"
...
if ($runningClient) {
    Write-Host "An Echo Warrior development client is already running (PID $($runningClient.ProcessId))."
...
$gradleArguments = @(':neoforge:runClient', '--console=plain')
if (Test-Path -LiteralPath $worldPath) {
    $gradleArguments += "-PquickPlayWorldNeoForge=$TestWorldName"
    Write-Host "Launching Echo Warrior NeoForge and entering $TestWorldName..."
} elseif ($RequireExistingWorld) {
    throw "The requested NeoForge test world does not exist: $worldPath"
} else {
    Write-Host "Launching Echo Warrior NeoForge. Create the world '$TestWorldName' once; later launches will enter it automatically."
}
